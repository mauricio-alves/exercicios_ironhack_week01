...
const moviesTitles = movies.map((currentElement) => {
...
const titlesAndOverviews = movies.map((currentElement) => {
  return {
    original_title: currentElement.original_title,
    overview: currentElement.overview,
  };
});
console.log(titlesAndOverviews);
...
const moreThanSeven = movies.map((currentElement) => {
  if (currentElement.vote_average >= 7) {
    return `O ${currentElement.original_title} é bom!`;
  } else {
    return `O ${currentElement.original_title} é ruim!`;
  }
});
console.log(moreThanSeven);

// Retornar uma nova array com apenas os filmes que tem nota 7.5 ou mais
const filterIfMoreThanSeven = movies.filter((currentElement) => {
  return currentElement.vote_average > 7.5;
});
console.log(filterIfMoreThanSeven);

// Retornar uma nova array que tenha apenas filmes lançados depois de 01/2022
const moviesDate = movies.filter((currentElement) => {
  return currentElement.release_date > "2022-01-01";
  // abaixo uma melhor forma de resolver (mais dinâmica):
  // return currentElement.release_date.includes("2022");
});
console.log(moviesDate);

// Retornar uma array com os filmes que tenham nota maior que 7.5 E apenas os titulos
const names = filterIfMoreThanSeven.map((currentElement) => {
  return currentElement.original_title;
});
